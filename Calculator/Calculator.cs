...
namespace Calculator
{
    ...
    public class Calculator
    {
        ...
        public double? Calculate()
        {
            ...
            List<String> tokenList = Regex.Split(_input, TokenSplitRegex,RegexOptions.IgnoreCase).Select(t => t.Trim()).Where(t => !String.IsNullOrEmpty(t)).ToList();
            for (int tokenNum = 0; tokenNum < tokenList.Count(); ++tokenNum)
            {
                double? tmpValue;
                String token = tokenList[tokenNum].ToUpper();
                ...
                switch (tokenType)
                {
                    case TokenType.Value:
                        if (tmpValue.HasValue)
                        {
                            if (!token.Contains(","))
                                ...
                                outputQueue.Enqueue(new Operand(token));
                        }
                        else
                    ...
                    case TokenType.Function:
                        ...
                        {
                            Function.FunctionTypes type = Function.GetFunctionType(token.ToLower());
            ...
            return (Double)outputStack.Pop().Value;
        }
